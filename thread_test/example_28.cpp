...
/*
Even though a customized copy constructor can help us to avoid data races, it is
also time (and memory) consuming. In the following, we will use move semantics
to implement a more effective way of safely passing data to a thread.
...
To make use of move semantics, we need to provide a move constructor (and
optionally a move assignment operator). Copy and assignment operations whose
sources are rvalues automatically take advantage of move semantics. Unlike the
default copy constructor however, the compiler does not provide a default move
constructor.

To define a move constructor for a C++ class, the following steps are required:

    Define an empty constructor method that takes an rvalue reference to the
class type as its parameter

    Image

    In the move constructor, assign the class data members from the source
...
- which calls the move constructor and invalidates the original object v0 in the
main thread.
*/