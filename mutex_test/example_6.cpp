...
/*
In this version of the code, std::lock_guard has been replaced with
std::unique_lock. As before, the lock object lck will unlock the mutex in its
destructor, i.e. when the function divideByNumber returns and lck gets out of
scope. In addition to this automatic unlocking, std::unique_lock offers the
...
for access to a locked resource. In the example, the lock is released before
