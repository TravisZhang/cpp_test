  ...
  while (true) {
    ...
    if (timeSinceLastUpdate >= cycleDuration) {
      ...
      if (completion >= 0.9 && !hasEnteredIntersection) {
        ...
        // to the method Intersection::addVehicleToQueue,
        // the object _currDestination and a shared pointer to this using the
        // get_shared_this() function.
        // Then, wait for the data to be available before proceeding to slow
        // down.
        std::shared_ptr<Vehicle> v_ptr = get_shared_this();
        std::future<void> ftr = std::async(
            [v_ptr](std::shared_ptr<Intersection> des) {
              des->addVehicleToQueue(v_ptr);
            },
            _currDestination);
        ftr.wait();
